...
public abstract class MixinProjectileExplosion{
    ...
    private void wrappedExplode(Operation<Void> original) {
        original.call();
        ...
        ShipUtilsKt.getLoadedShipsIntersecting(this.level, AABBdUtilKt.expand(new AABBd(this.x, this.y, this.z, this.x, this.y, this.z), this.radius)).forEach(ship -> {
            ship.getTransform().getWorldToShip().transformPosition(this.x, this.y, this.z, tmp);
            this.x = tmp.x;
            this.y = tmp.y;
            this.z = tmp.z;
            original.call();
        ...
        for (int x = radius; x >= -radius; x--) {
            for (int y = radius; y >= -radius; y--) {
                for (int z = radius; z >= -radius; z--) {
                    final BlockHitResult result = this.level.clip(
